...
public sealed class SldlApiClient
{
    ...
    public SldlApiClient(HttpClient http, JsonSerializerOptions? jsonOptions = null)
    {
        this.http = http;
    ...
    private static string? TryReadApiError(string body)
    {
        try
        {
            return JsonSerializer.Deserialize<ApiErrorDto>(body, SldlApiJson.CreateSerializerOptions())?.Error;
        }
        catch (JsonException)
        {
            return null;
